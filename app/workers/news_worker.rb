        ...
        if nyt_parsed_data['response'].present?
          begin
            nyt_parsed_data['response']['docs'].first(50).each do |nyt_data|
              NewsArticle.find_or_initialize_by(search_term: nyt_data['headline']['main'],headline: nyt_data['headline']['main'], url: nyt_data['web_url'], text: nyt_data['snippet'], author: nyt_data['byline'].present? ? nyt_data['byline']['organization'] : nil, api_type: 0, date_article: nyt_data['pub_date'].present? ? nyt_data['pub_date'] : nil) do |user|
               user.save!
              end  
            end  
          rescue => e
