module SeoPagesHelper
  ...
  def historial_rates_report(postal_codes, loan_purpose)
    ...
    complete_data =  if loan_purpose == 'P'
      FreddieMacLoanOrigination.where(postal_code: postal_codes, loan_purpose: loan_purpose).where("first_payment_date >= ? and first_payment_date <= ?", start_year.beginning_of_year, end_year.end_of_year)
    else 
