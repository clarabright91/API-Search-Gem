class FdicInstitution < ApplicationRecord
  ...
  def self.search_term(prefix)
    where("lower(name) like ? and active = '1'", "#{prefix.downcase}%").sort_by(&:name)
  end
  ...
  def self.search_term_special
    where("name !~ ? and active = '1'", "^[A-Za-z]").sort_by(&:name)
  end
  ...
  def self.near_by_banks(cert)
    first_two_banks = FdicInstitution.where("cert > ? and active = '1'", cert).order(cert: :ASC).limit(2)
    last_three_banks = FdicInstitution.where("cert < ? and active = '1'", cert).order(cert: :DESC).limit(3)
